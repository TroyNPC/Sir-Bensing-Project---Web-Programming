...
public function new(Request $request, EntityManagerInterface $entityManager, SluggerInterface $slugger): Response
{
    ...
    if ($form->isSubmitted() && $form->isValid()) {
        ...
        // ✅ Automatically create a new InventoryLog for this product
        $inventoryLog = new \App\Entity\InventoryLog();
        $inventoryLog->setProductname($pcproduct);
        $inventoryLog->setStock(1); // Default stock 1
        $inventoryLog->setImage($pcproduct->getImage());
        $inventoryLog->setCreatedAt(new \DateTimeImmutable());

        // ✅ Adjust InventoryLog AUTO_INCREMENT
        $maxLogId = $connection->fetchOne('SELECT MAX(id) FROM inventory_log');
        $nextLogId = ((int)$maxLogId) + 1; // If table empty → 1
        $connection->executeStatement('ALTER TABLE inventory_log AUTO_INCREMENT = ' . $nextLogId);

        // ✅ Persist inventory log
        $entityManager->persist($inventoryLog);
        $entityManager->flush();

        // ✅ Flash success message
        $this->addFlash('success', '✅ Product added successfully and inventory log created!');
